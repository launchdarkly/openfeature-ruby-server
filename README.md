...
| ✅      | Eventing                        | LaunchDarkly data source status changes are emitted as `PROVIDER_READY`, `PROVIDER_STALE`, and `PROVIDER_ERROR`, once initialization has completed and only when the status changes. Flag changes are emitted as `PROVIDER_CONFIGURATION_CHANGED` with the changed flag key. |
